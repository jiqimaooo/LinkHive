...
python3 scripts/build_deploy_package.py \
  --output dist/LinkHive-deploy.zip
```

`scripts/install_latest.sh` 用于从 GitHub Release 下载部署包并执行安装。GitHub Actions 会自动构建前端、同步到 `deploy/web_admin/frontend_dist`、生成部署 zip，并生成 `lpac-assets.json`。
