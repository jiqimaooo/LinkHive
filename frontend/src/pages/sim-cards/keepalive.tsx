...
export default function KeepalivePage() {
  const { status, actionBusy, keepaliveSettings, setKeepaliveSettings, keepaliveTasks, setKeepaliveTasks, expandedKeepaliveTaskId, setExpandedKeepaliveTaskId, saveKeepalive, sendKeepaliveTestSms, runAction, keepaliveDirtyRef } = useAppContext()
  const keepalive = getKeepalive(status)
  const devices = status?.devices ?? []
  const keepaliveEnabledCount = keepalive.tasks.filter((t) => t.enabled).length

  const descText = "按设备配置定时短信保活。普通 SIM 直接发送，eSIM 设备可选择 Profile 后执行切卡保活。"
  const emptyText = "添加任务后先选择目标设备，再配置 cron、目标号码和短信内容。"

  return (
    <div className="space-y-4">
      <PageHeader title="短信保活" description={descText} />
        ...
        <Card>
          <CardHeader>
            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              ...
              <Button type="button" size="sm" variant="outline" disabled={actionBusy || !devices.length} onClick={() => { const nt = createKeepaliveTask(devices, status?.profiles ?? []); keepaliveDirtyRef.current = true; setKeepaliveTasks((c) => [...c, nt]); setExpandedKeepaliveTaskId(nt.id) }}><PlusIcon data-icon="inline-start" />添加任务</Button>
            </div>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="glass-panel rounded-xl p-4">
              <div className="grid gap-4 md:grid-cols-[220px_1fr] md:items-end">
                <div className="grid gap-2"><Label htmlFor="kg">队列缓冲时间（秒）</Label><Input id="kg" type="number" min={30} max={1800} value={String(keepaliveSettings.queue_gap_seconds)} onChange={(e) => { keepaliveDirtyRef.current = true; const v = Number.parseInt(e.target.value, 10); setKeepaliveSettings(() => ({ queue_gap_seconds: Number.isNaN(v) ? 180 : v })) }} /></div>
                <p className="text-sm text-muted-foreground">多个任务同时到点时会自动排队；eSIM 任务会用这个间隔避免频繁切卡。</p>
              </div>
            </div>
            ...
            {keepaliveTasks.length ? <div className="flex flex-col gap-4">
              {keepaliveTasks.map((task) => {
                ...
                const deviceProfiles = (status?.profiles ?? []).filter((profile) => !task.device_id || profile.device_id === task.device_id)
                const isEsimDevice = Boolean(device?.capabilities.esim_supported)
                const profileName = isEsimDevice
                  ? (saved?.profile_name || deviceProfiles.find((p) => p.iccid === task.profile_iccid)?.display_name || "可直接使用当前 SIM")
                  : null
                    ...
                    <div className="flex flex-col gap-4">
                      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                        <div className="flex min-w-0 flex-1 flex-wrap items-center gap-2">
                          ...
                          <Badge variant={device ? "outline" : "destructive"}>{device?.label || "未绑定设备"}</Badge>
                          {isEsimDevice && profileName ? <Badge variant="outline">{profileName}</Badge> : null}
                      ...
                      {isExpanded ? <>
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          <Button type="button" size="sm" variant="outline" disabled={actionBusy || !saved || !task.device_id} onClick={() => { void runAction("run_keepalive_task", { task_id: task.id, trigger: "manual", device_id: task.device_id }, `执行保活 ${task.label}`) }}><SendIcon data-icon="inline-start" />立即执行</Button>
                          <Button type="button" size="sm" variant="outline" disabled={actionBusy} onClick={() => { keepaliveDirtyRef.current = true; setKeepaliveTasks((c) => c.filter((t) => t.id !== task.id)) }}><Trash2Icon data-icon="inline-start" />删除</Button>
                            ...
                            <div className="grid gap-2"><Label>目标设备</Label><Select value={task.device_id} onValueChange={(v) => { keepaliveDirtyRef.current = true; setKeepaliveTasks((c) => c.map((t) => t.id === task.id ? { ...t, device_id: v ?? "", profile_iccid: "" } : t)) }}><SelectTrigger><SelectValue placeholder="选择设备" /></SelectTrigger><SelectContent><SelectGroup><SelectLabel>设备</SelectLabel>{devices.map((d) => <SelectItem key={d.id} value={d.id}>{d.label}</SelectItem>)}</SelectGroup></SelectContent></Select></div>
	                          {isEsimDevice ? (
	                            <div className="grid gap-2 md:col-span-2"><Label>目标 Profile</Label><Select value={task.profile_iccid || "__current__"} onValueChange={(v) => { keepaliveDirtyRef.current = true; setKeepaliveTasks((c) => c.map((t) => t.id === task.id ? { ...t, profile_iccid: v === "__current__" ? "" : v ?? "" } : t)) }}><SelectTrigger><SelectValue placeholder="选择 Profile" /></SelectTrigger><SelectContent><SelectGroup><SelectLabel>Profiles</SelectLabel><SelectItem value="__current__">不切卡，使用当前 SIM</SelectItem>{deviceProfiles.map((p) => <SelectItem key={p.iccid} value={p.iccid}>{p.display_name}</SelectItem>)}</SelectGroup></SelectContent></Select></div>
	                          ) : null}
	                          <div className={cn("grid gap-2", isEsimDevice ? "md:col-span-2" : "")}><Label htmlFor={`kc-${task.id}`}>cron 表达式</Label><Input id={`kc-${task.id}`} value={task.cron_expression} onChange={(e) => { keepaliveDirtyRef.current = true; setKeepaliveTasks((c) => c.map((t) => t.id === task.id ? { ...t, cron_expression: e.target.value } : t)) }} placeholder="例如 0 9 * * *" /><p className="text-sm text-muted-foreground">5 段 cron：分钟 小时 日 月 星期。示例：0 9 * * * = 每天 09:00。</p></div>
	                          <div className={cn("grid gap-2", !isEsimDevice ? "md:col-span-2" : "")}><Label htmlFor={`kt-${task.id}`}>目标号码</Label><Input id={`kt-${task.id}`} value={task.target_number} onChange={(e) => { keepaliveDirtyRef.current = true; setKeepaliveTasks((c) => c.map((t) => t.id === task.id ? { ...t, target_number: e.target.value } : t)) }} placeholder="例如 +447000000000" /></div>
                        </div>
          ...
          <CardContent className="space-y-3">
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <div className="glass-panel rounded-xl p-4 text-center">
                <div className="text-sm text-muted-foreground">下次可执行</div>
