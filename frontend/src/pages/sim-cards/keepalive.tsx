...
export default function KeepalivePage() {
  ...
  return (
    <div className="space-y-4">
      <PageHeader icon={Clock3Icon} title="保活任务" description="通过 cron 表达式定时自动切卡、发短信、回切，确保 SIM 卡保持活跃。" />

      {!esimEnabled ? (
        <div className="rounded-2xl border border-dashed border-border/70 bg-background/70 px-6 py-10 text-center"><p className="text-sm text-muted-foreground">当前为普通 SIM 模式，保活任务依赖 Profile 切换，请先切换到 eSIM 模式。</p></div>
      ) : (
          ...
          <Card className="border-slate-200 bg-white">
            <CardHeader>
              <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                ...
                <Button type="button" size="sm" variant="outline" disabled={actionBusy || !status?.profiles.length} onClick={() => { const nt = createKeepaliveTask(status?.profiles ?? []); keepaliveDirtyRef.current = true; setKeepaliveTasks((c) => [...c, nt]); setExpandedKeepaliveTaskId(nt.id) }}><PlusIcon data-icon="inline-start" />添加任务</Button>
              </div>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              {keepaliveTasks.length ? <div className="flex flex-col gap-4">
                {keepaliveTasks.map((task) => {
                  ...
                  const profileName = saved?.profile_name || (status?.profiles ?? []).find((p) => p.iccid === task.profile_iccid)?.display_name || "待选择 Profile"
                    ...
                    <div key={task.id} className="rounded-2xl border bg-white/80 p-4 shadow-sm">
                      <div className="flex flex-col gap-4">
                        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                          <div className="flex min-w-0 flex-1 flex-wrap items-center gap-2"><span className="text-sm font-medium">{task.label || "未命名任务"}</span><Badge variant="outline">{profileName}</Badge><Badge variant="secondary">{task.enabled ? "已启用" : "已停用"}</Badge><Badge variant="outline">{saved?.schedule_label || task.cron_expression || "--"}</Badge>{saved?.next_run_label ? <Badge variant="outline">{saved.next_run_label}</Badge> : null}</div>
                        ...
                        {isExpanded ? <>
                          ...
                          <div className="grid gap-4 md:grid-cols-2">
                            ...
                            <div className="grid gap-2 md:col-span-2"><Label htmlFor={`kc-${task.id}`}>cron 表达式</Label><Input id={`kc-${task.id}`} value={task.cron_expression} onChange={(e) => { keepaliveDirtyRef.current = true; setKeepaliveTasks((c) => c.map((t) => t.id === task.id ? { ...t, cron_expression: e.target.value } : t)) }} placeholder="例如 0 9 * * *" /><p className="text-sm text-muted-foreground">5 段 cron：分钟 小时 日 月 星期。示例：0 9 * * * = 每天 09:00。</p></div>
                            <div className="grid gap-2"><Label htmlFor={`kt-${task.id}`}>目标号码</Label><Input id={`kt-${task.id}`} value={task.target_number} onChange={(e) => { keepaliveDirtyRef.current = true; setKeepaliveTasks((c) => c.map((t) => t.id === task.id ? { ...t, target_number: e.target.value } : t)) }} placeholder="例如 +447000000000" /></div>
                          </div>
              ...
              </div> : <EmptyState icon={Clock3Icon} title="还没有保活任务" description="添加任务后即可按 cron 表达式自动切卡、发短信、通知并回切。" />}
          ...
          <Card className="border-slate-200 bg-white">
            ...
            <CardContent className="space-y-3">
              <div className="grid gap-3 sm:grid-cols-2">
                ...
                <div className="rounded-xl border p-4 text-center"><div className="text-sm text-muted-foreground">下次可切卡</div><div className="mt-2 text-sm font-medium">{keepalive.next_allowed_at || "当前可执行"}</div></div>
              </div>
              <div className="rounded-xl border p-4">
                ...
                {keepalive.active_run ? <div className="space-y-2 text-sm text-muted-foreground"><div>{keepalive.active_run.label}</div><div>触发：{keepaliveTriggerLabel(keepalive.active_run.trigger)}</div><div>Profile：{keepalive.active_run.profile_name || "--"}</div><div>计划：{keepalive.active_run.scheduled_for_label || "--"}</div><div className="whitespace-pre-wrap text-foreground/80">{keepalive.active_run.last_message || "任务已启动..."}</div></div> : <p className="text-sm text-muted-foreground">当前没有保活任务正在执行。</p>}
              </div>
              <div className="rounded-xl border p-4">
